...
// ---------- Netlify _redirects: portal deep links → our detail pages ----------
// Inmovilla tells portals (idealista etc.) that a listing lives at
//   https://aldeiarealty.com/ficha/index.php?codigo=14306_<inmovilla id>
// That is their PHP-template URL; on our static site it 404s. So every
// property we know about (live, archived, pending) gets a 301 to its real
// page — Portuguese-language browsers to the PT page, everyone else to EN —
// and anything unknown under /ficha/ falls back to the listings page.
function writeRedirects(allProps) {
  const lines = [
    '# Generated by scripts/build-listings.js — do not hand-edit this block.',
    '# Inmovilla/idealista deep links (ficha/index.php?codigo=14306_<id>) → real pages.',
  ];
  for (const p of allProps) {
    if (!p.id || !p.slug) continue;
    const q = 'codigo=14306_' + p.id;
    lines.push(`/ficha/index.php ${q} /pt/imovel-${p.slug}.html 301 Language=pt`);
    lines.push(`/ficha/index.php ${q} /property-${p.slug}.html 301`);
    lines.push(`/ficha/ ${q} /property-${p.slug}.html 301`);
  }
  lines.push('/ficha/* /pt/properties.html 302 Language=pt');
  lines.push('/ficha/* /properties.html 302');
  lines.push('');
  fs.writeFileSync(path.join(ROOT, '_redirects'), lines.join('\n'));
  console.log('Wrote _redirects (' + allProps.length + ' properties)');
}
...
async function main() {
  ...
  writeRedirects([...props, ...archived, ...pendingProps]);
  console.log('Done.');
}
